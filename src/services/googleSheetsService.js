...
export async function getSheetsClient(spreadsheetId = config.spreadsheetId) {
  ...
  const client = google.sheets({ version: 'v4', auth: getGoogleAuth() });
...
export async function appendRow(sheetName, values, spreadsheetId = config.spreadsheetId) {
  ...
  const res = await withRetry(() => sheets.spreadsheets.values.append({
    spreadsheetId, range, valueInputOption: 'USER_ENTERED', insertDataOption: 'INSERT_ROWS', requestBody: { values: [values] }
  }), `appendRow:${sheetName}`);
...
export async function appendRows(sheetName, rows, spreadsheetId = config.spreadsheetId) {
  ...
  const range = `'${sheetName}'!A:Z`;
  const res = await withRetry(() => sheets.spreadsheets.values.append({
    spreadsheetId, range, valueInputOption: 'USER_ENTERED', insertDataOption: 'INSERT_ROWS', requestBody: { values: rows }
...
export async function readRange(sheetName, a1Range = 'A:Z', spreadsheetId = config.spreadsheetId) {
  const sheets = await getSheetsClient(spreadsheetId);
  const range = `'${sheetName}'!${a1Range}`;
  const res = await withRetry(() => sheets.spreadsheets.values.get({ spreadsheetId, range }), `readRange:${sheetName}!${a1Range}`);
...
export async function ensureSheetHeaders(headersMap, spreadsheetId = config.spreadsheetId) {
  ...
  Object.entries(headersMap).forEach(([title, headers], index) => {
    if (!existing.has(title)) {
      requests.push({ addSheet: { properties: { title, index: index + 1, gridProperties: { rowCount: 500, columnCount: Math.max(headers.length, 12), frozenRowCount: 1 } } } });
    }
  });
  if (requests.length) await withRetry(() => sheets.spreadsheets.batchUpdate({ spreadsheetId, requestBody: { requests } }), 'batchUpdate:addSheets');
  for (const [title, headers] of Object.entries(headersMap)) {
    await withRetry(() => sheets.spreadsheets.values.update({
      spreadsheetId, range: `'${title}'!A1:${columnLetter(headers.length)}1`, valueInputOption: 'USER_ENTERED', requestBody: { values: [headers] }
    }), `headers:${title}`);
  }
}

function columnLetter(n) { let s = ''; while (n > 0) { const m = (n - 1) % 26; s = String.fromCharCode(65 + m) + s; n = Math.floor((n - m - 1) / 26); } return s; }

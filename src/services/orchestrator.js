...
import { createEvent, createContentTasks, getFeedbackRules, getRecentContentTasks, saveSystemLog } from './sheetsStorage.js';
...
export async function processUserText({ text, messageMeta, runLogger }) {
  ...
  if (route.intent === 'analyze_storylines') {
    const storyline = await analyzeStorylineFromText({ text });
    ...
    return { type: 'storyline', textRu: `Нашёл возможный storyline: ${storyline.trigger_type}.\nЗаписал во вкладку Storylines. Пока это idea/review, не автопостинг.` };
  }

  if (route.intent === 'create_event_campaign') {
    ...
    await saveSystemLog({ run_id: runId, level: 'INFO', agent: 'Content Planner Agent', action: 'create_event_campaign', status: 'Created', output_summary: `${createdTasks.length} tasks created`, raw_json: { plan, createdEvent, createdTasks, drafts } });

    return { type: 'event_campaign', textRu: formatEventCampaignReply({ createdEvent, createdTasks, drafts, plan }) };
  }
...
function formatEventCampaignReply({ createdEvent, createdTasks, drafts, plan }) {

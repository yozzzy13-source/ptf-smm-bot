...
export function referenceBatchKeyboard(eventId = 'active') {
  const id = compactId(eventId);
  return {
    inline_keyboard: [
      [
        { text: '✅ Подтвердить референсы', callback_data: `ptf|approve|refbatch|${id}` }
      ],
      [
        { text: '🎨 Главный постер 4:5', callback_data: `ptf|generate|visual|${id}` },
        { text: '📱 Story 9:16', callback_data: `ptf|generate_story|visual|${id}` }
      ],
      [
        { text: '🖼 Telegram cover 16:9', callback_data: `ptf|generate_tg|visual|${id}` }
      ]
    ]
  };
}

export function visualSetKeyboard(visualJobId) {
  const id = compactId(visualJobId);
  return {
    inline_keyboard: [
      [
        { text: '✅ Утвердить 1', callback_data: `ptf|approve_v1|vjob|${id}` },
      ...
      [
        { text: '🔁 Перегенерировать оба', callback_data: `ptf|regen_both|vjob|${id}` }
      ]
    ]
  };
}
...
export function campaignSelectorKeyboard(campaigns = []) {
  return {
    inline_keyboard: campaigns.slice(0, 8).map((c) => [
      { text: campaignButtonLabel(c), callback_data: `ptf|focus|cmp|${compactId(c.event_id)}` }
    ])
  };
}

function campaignButtonLabel(c = {}) {
  return `${c.player1 || 'Event'} vs ${c.player2 || ''} · ${shortDate(c.date)}`.slice(0, 60);
}
...
function compactId(v) {
  return String(v || '').replace(/[^A-Za-z0-9_-]/g, '').slice(0, 48) || 'unknown';
}
